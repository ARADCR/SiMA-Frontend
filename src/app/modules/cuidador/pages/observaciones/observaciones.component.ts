...
export class ObservacionesComponent implements OnInit {
  ...
  obsFiltradas = computed(() => {
    ...
    if (q) {
      ...
      list = list.filter(o => o.texto.toLowerCase().includes(qLower) || this.nombrePaciente(o.idAdulto).toLowerCase().includes(qLower));
    }
    const pf = this.pacienteFiltro();
    if (pf) list = list.filter(o => String(o.idAdulto) === pf);
    const uf = this.urgenciaFiltro();
    if (uf) list = list.filter(o => o.urgencia === uf);
    return list;
  });
